...
int main(int argc, char* argv[])
{
    ...
    // Coordenadas iniciais da câmera
    float x = 0.0f;
    float y = 0.0f;
    float z = -5.0f;
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        float y_view = r*sin(g_CameraPhi);
        float z_view = r*cos(g_CameraPhi)*cos(g_CameraTheta);
        float x_view = r*cos(g_CameraPhi)*sin(g_CameraTheta);

        glm::vec4 camera_position_c  = glm::vec4(x,y,z,1.0f); // Ponto "c", centro da câmera
        glm::vec4 camera_view_vector = glm::vec4(x_view,y_view,z_view,0.0f); // Vetor "view", sentido para onde a câmera está virada
